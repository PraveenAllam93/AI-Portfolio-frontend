<script lang="ts">
	...
	onMount(async () => {
		if (!navState.question) {
			try {
				...
				// user sees empty question, error surfaces on submit
			}
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		// Advance to next question immediately — no per-question feedback shown
		if (result.data.nextQuestion) {
			question = result.data.nextQuestion.question;
			questionNumber = result.data.nextQuestion.questionNumber;
			currentTopic = result.data.nextQuestion.topic;
			answer = '';
		}
		...
		{:else}
			...
			<form onsubmit={handleSubmit} class="space-y-4">
				...
				>
					{#if isSubmitting}
						<Spinner size="sm" />
						<span class="ml-2">{questionNumber === totalQuestions ? 'Finishing…' : 'Next Question…'}</span>
					{:else}
						{questionNumber === totalQuestions ? 'Finish Interview' : 'Submit & Continue'}
					{/if}
				</button>
			</form>
		{/if}
