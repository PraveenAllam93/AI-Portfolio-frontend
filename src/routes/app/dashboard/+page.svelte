...
<div class="flex min-h-screen flex-col bg-[#fafafa]">
			...
			<div class="flex items-center gap-4">
				{#if $authStore.user}
					<div class="flex items-center gap-3 border-r border-slate-200 pr-4">
						<div
							class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-700"
						>
							{userInitials}
						</div>
	...
	<main class="mx-auto w-full max-w-5xl flex-1 px-6 py-12">
		...
		{#if portfolioUrlLoading}
			<div class="flex items-center gap-3 py-12 text-sm text-slate-500 font-medium justify-center">
		...
		{:else if portfolioUrl}
			...
			<div use:reveal={{ delay: 50 }} class="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
				{#each [{ label: 'Status', value: 'Live', valueClass: 'text-emerald-600', icon: 'M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z', iconColor: 'text-emerald-500' }, { label: 'Template', value: 'Clean', valueClass: 'text-slate-900', icon: 'M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25Z', iconColor: 'text-blue-500' }, { label: 'Total Views', value: analyticsLoading ? '…' : totalViews !== null ? totalViews.toLocaleString() : '–', valueClass: 'text-slate-900', icon: 'M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z', iconColor: 'text-purple-500' }, { label: 'Last Updated', value: 'Recent', valueClass: 'text-slate-600', icon: 'M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z', iconColor: 'text-slate-400' }] as stat}
					<div class="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 {stat.iconColor}">
			...
			<!-- Portfolio card -->
			<div use:reveal={{ delay: 100 }} class="overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
				<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
					<div class="flex items-start gap-5">
						<div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-slate-50 border border-slate-100 shadow-inner">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8 text-slate-900">
						...
						<div class="min-w-0">
							<div class="flex items-center gap-3">
								...
								<div class="flex items-center gap-2">
									<span class="rounded-full bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 text-xs font-bold text-emerald-600 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>Live</span>
									{#if activeVersion}
										<span class="rounded-full bg-slate-100 border border-slate-200 px-2.5 py-0.5 text-xs font-bold text-slate-600 capitalize">{activeVersion}</span>
									{/if}
								</div>
							</div>
							<p class="mt-2 text-sm text-slate-500">Your site is up and running securely on the edge network.</p>
							<div class="mt-4 flex items-center gap-2">
                                <p class="max-w-xs truncate rounded-lg bg-slate-50 border border-slate-100 px-3 py-1.5 font-mono text-sm font-medium text-slate-700">{portfolioUrl}</p>
                            </div>
						</div>
					</div>
					<div class="flex shrink-0 flex-col gap-3 sm:flex-row">
                        {#if $authStore.user}
							<a href="/app/portfolio/{$authStore.user.userId}/analytics" class="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 hover:text-slate-900">Analytics</a>
							<a href="/app/portfolio/{$authStore.user.userId}/edit" class="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 hover:text-slate-900">Edit Content</a>
						{/if}
						<a href={portfolioUrl} target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 rounded-full bg-slate-900 px-6 py-2.5 text-sm font-bold text-white shadow-lg transition-all hover:bg-slate-800 active:scale-95">
							Open Link
		...
		{:else}
			...
			<div use:reveal={{ delay: 100 }} class="flex flex-col items-center justify-center rounded-[2.5rem] border border-slate-200 bg-white px-8 py-24 text-center shadow-sm">
				<div class="mb-6 flex h-20 w-20 items-center justify-center rounded-2xl bg-slate-50 border border-slate-100">
