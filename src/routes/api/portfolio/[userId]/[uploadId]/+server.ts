...
export const GET: RequestHandler = async ({ params, cookies }) => {
	...
	try {
		upstream = await fetch(`${apiBase}/portfolio/${params.userId}/${params.uploadId}`, {
...
export const DELETE: RequestHandler = async ({ params, cookies }) => {
	...
	const upstream = await fetch(`${apiBase}/portfolio/${params.userId}/${params.uploadId}`, {
		method: 'DELETE',
		headers: { Authorization: `Bearer ${idToken}` }
	});
	...
	if (!upstream.ok) {
		...
		throw error(upstream.status, (err as { error?: string }).error ?? 'Failed to delete portfolio');
	}
	return json(await upstream.json());
};
