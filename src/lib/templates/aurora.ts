...
<div class="card-title" ${ed(`campaigns.${i}.campaign_name`)}>${c.campaign_name || ''}</div>${(c.campaign_type) ? `<div class="cert-meta">&mdash; <span ${ed(`campaigns.${i}.campaign_type`)}>${c.campaign_type || ''}</span></div>` : ''}
...
${(ip.assets_under_management) ? `<span><span style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);display:block">AUM</span><strong ${ed(`investment_portfolios.${i}.assets_under_management`)}>${ip.assets_under_management || ''}</strong></span>` : ''}
${(ip.performance_return) ? `<span><span style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);display:block">Return</span><strong style="color:var(--coral)" ${ed(`investment_portfolios.${i}.performance_return`)}>${ip.performance_return || ''}</strong></span>` : ''}
</div>
</div>`;
  }
